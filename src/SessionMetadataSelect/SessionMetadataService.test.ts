...
describe("Session Metadata Service", () => {
    ...
    it("should handle a successful request with a single element in the response", async function () {
        ...
        apiMock.onGet("/sessions/metadata").reply(200, JSON.stringify(Array.of(firstSessionMetadata)))
    ...
    it("should handle a successful request with a multiple elements in the response", async function () {
        ...
        apiMock.onGet("/sessions/metadata").reply(200, JSON.stringify(Array.of(firstSessionMetadata, secondSessionMetadata)))

        const response = await sessionMetadataService.getAllSessionMetadata();

        expect(response.status).toBe(200)
        expect(response.data).toStrictEqual(Array.of(firstSessionMetadata, secondSessionMetadata))
    });
